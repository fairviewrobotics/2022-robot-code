...
class RobotContainer {
    ...
    private fun configureButtonBindings() {
        ...
        val runIntakeIndexer = { forward: Boolean ->
            if (forward) {
                FixedBallMotorSpeed(intakeIndexer) { Constants.indexerSpeed }
            } else {
                FixedBallMotorSpeed(intakeIndexer) { -Constants.indexerSpeed }
            }
        }
...
//        }
        ...
        JoystickButton(secondaryController, kA.value).whenHeld(runGate(true))
        JoystickButton(secondaryController, kB.value).whenHeld(runGate(false))
