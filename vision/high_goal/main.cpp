...
static const char* configFile = "/boot/frc_high_goal.json";
  ...
  if (cameras.size() >= 1) {
    std::thread([&] {
      frc::VisionRunner<HighGoalPipeline> runner(cameras[0], new HighGoalPipeline(),
        ...
        if(pipeline.found_target) {
          table->GetEntry("yaw").SetDouble(pipeline.yaw);
          table->GetEntry("pitch").SetDouble(pipeline.pitch);
          table->GetEntry("center_distance").SetDouble(pipeline.center_distance);
        }
      });
