...
class Tester:
    def __init__(self, config_parser):
        ...
        try:
            model_path = "model_tf2_edgetpu.tflite"
        ...
        self.ntinst.startClientTeam(config_parser.team)
        self.ntinst.startDSClient()
        self.entry = self.ntinst.getTable("ML").getEntry("detections")

        self.coral_entry = self.ntinst.getTable("ML").getEntry("coral")
        self.fps_entry = self.ntinst.getTable("ML").getEntry("fps")

        self.coral_entry.setString("Hello!")
        print(self.ntinst.getTable("ML").getEntry("coral").getString("uh oh"))
        self.ntinst.flush()

        self.ntinst.enableVerboseLogging()


        self.resolution_entry = self.ntinst.getTable("ML").getEntry("resolution")
        self.temp_entry = []
        ...
        self.coral_entry.setString("lahsflakjhf")
        ...
        while True:
            ...
            for i, box in enumerate(boxes):
                if scores[i] > .4:
                    ...
                    frame_cv2 = self.label_frame(frame_cv2, self.labels[class_id], box, scores[i], x_scale,
                                                 y_scale)
            self.output.putFrame(frame_cv2)
            self.entry.setString(json.dumps(boxes.tolist()))
            self.temp_entry = []
    ...
    def set_input(self, frame):
        ...
        h, w, _ = frame.shape
        new_img = np.reshape(cv2.resize(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), (300, 300)), (1, 300, 300, 3))
